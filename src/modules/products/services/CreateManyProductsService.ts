import prismaClient from "../../../prisma"
import { sanitizeProduct } from "../../../utils/sanitizeProducts";

interface CreateProductProps {
    code: string,
    description: string
}

interface ProcessResult {
    created: CreateProductProps[];
    skipped: CreateProductProps[];
    errors: Array<{
        code: string;
        description: string;
        error: string;
    }>;
}

class CreateManyProductsService {
    async execute(products: CreateProductProps[], userId: string): Promise<ProcessResult> {
        const result: ProcessResult = {
            created: [],
            skipped: [],
            errors: []
        };

        /* Sanitizar os produtos antes de qualquer interação com o banco de dados,
garantindo que code seja um inteiro e descrição esteja correta */

        const sanitizedProducts: CreateProductProps[] = products.map(product => sanitizeProduct(product));

        const codes = sanitizedProducts.map(p => p.code);

        const hasDuplicates = new Set(codes).size !== codes.length

        if (hasDuplicates) {
            const duplicates = codes.filter((code, index, self) =>
                self.indexOf(code) !== index
            );

            throw new Error(`Há códigos repetidos no envio: ${duplicates.join(", ")}`)
        }

        const existing = await prismaClient.product.findMany({
            where: {
                code: { in: codes },
                userId: userId
            },
            select: { code: true },
        });
        const existingCodes = new Set(existing.map(p => p.code));

        const toCreate: CreateProductProps[] = [];

        for (const product of sanitizedProducts) {
            if (existingCodes.has(product.code)) {
                result.skipped.push(product);
            } else {
                toCreate.push(product);
            }
        }

        try {
            await prismaClient.product.createMany({
                data: toCreate.map(p => ({
                    code: p.code.replace(/^0+/, ''),
                    description: p.description,
                    userId
                })),
                skipDuplicates: true,
            });

            result.created.push(...toCreate);
        } catch (error: any) {
            for (const product of toCreate) {
                result.errors.push({
                    code: product.code,
                    description: product.description,
                    error: error.message || "Erro ao cadastrar produto",
                });
            }
        }

        return result;
    }
}

export { CreateManyProductsService }